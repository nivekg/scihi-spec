...
def get_data(r):
    ...
    rv = {}
    for pol in range(4):
        bram_name = 'pol%d%d' % (pol, pol)
        rv[bram_name]  = np.fromstring(r.read(bram_name, 8*NCHANS), dtype='>i8')
    for pol1 in range(4):
        for pol2 in range(pol1, 4):
            bram_name = 'pol%d%d' % (pol1,pol2)
            rv[bram_name]  = np.fromstring(r.read(bram_name+'r', 8*NCHANS), dtype='>i8') + 1j*np.fromstring(r.read(bram_name+'i', 8*NCHANS), dtype='>i8')
    return rv
...
if __name__ == '__main__':
    ...
    r.write_int('chan0_fft_shift', opts.fftshift & 0xffff)
    r.write_int('chan1_fft_shift', opts.fftshift & 0xffff)
    print 'Checking for FFT overflows...'
    oflow = False
    for i in range(5):
        oflow = oflow or bool(r.read_int('chan0_fft_of'))
        oflow = oflow or bool(r.read_int('chan1_fft_of'))
        time.sleep(1)
    if oflow:
